...
void reaper(){
    ...
    while ((ended=waitpid(-1, &status, WNOHANG)) > 0) {
        ...
        fprintf(stderr,"\n[reaper()→ Proceso hijo %d en background(%s) finalizado con exit code 0].\n",ended,jobs_list[pos].cmd);
...
int jobs_list_remove(int pos){
    ...
    //caso posició es igual al número de pids
    if (pos == n_pids) {
    jobs_list[pos].pid = 0;
    ...
    //decrementamos el contador de procesos no finalizados
    n_pids--;
    return 0;
}
