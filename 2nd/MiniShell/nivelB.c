...
int execute_line(char *line) {
    ...
    if (parse_args(args, line) > 0) {
        ...
        if(pid==0){//hijo
            ...
            execvp(args[0],args);
            fprintf(stderr, "[execute_line() --> No se ha ejecutado correctamente el execvp]\n");
            fprintf(stderr, "%s: no se encontró la orden\n", args[0]);
            exit(-1);

        }else if (pid>0) {//padre
            //le recordamos al padre que si se produce ctr+c o un hijo finaliz, ejecutemos el manejador 
            //ctrlc y repaer, respectivamente 
            signal(SIGINT, ctrlc);
...
void reaper(){
    ...
    pid_t ended;
    int status;
    //Analizamos el valor que devuelve waitpid para saber que hiho termina
...
void ctrlc(){
    ...
    if(jobs_list[0].pid > 0){
        if(strcmp(jobs_list[0].cmd,mi_shell)){
            ...
            fprintf(stderr, "[ctrlc()→ Señal 15 (SIGTERM) enviada a %d(%s) por %d(%s)]\n", jobs_list[0].pid, jobs_list[0].cmd, getpid(), mi_shell);
        }else{
            fprintf(stderr, "[ctrlc()→ Señal 15 (SIGTERM) NO enviada debido a que el proceso en foreground es el shell]\n");
        }
    }else{
        fprintf(stderr, "[ctrlc()→ Señal 15 (SIGTERM) no enviada debido a que no hay proceso en foreground]\n");
    }
