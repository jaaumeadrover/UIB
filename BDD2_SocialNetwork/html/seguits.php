<?php
    ...
    $string="SELECT idUsuariSeguit FROM Seguidor WHERE idUsuariSeguidor = '" . $idUsuari . "'";
    ...
    <?php 
        while($row=mysqli_fetch_array($consulta)){
            $string2 = "SELECT idUsuari,nom, cognom FROM Usuari WHERE idUsuari = '" . $row["idUsuariSeguit"] . "'";
            $consulta2 = mysqli_query($conexio,$string2);
            $row2 = mysqli_fetch_array($consulta2);
    ?>      
            <tr>
                <?php $cadena="perfilExtern.php?id=" . $row2["idUsuari"]; ?>
