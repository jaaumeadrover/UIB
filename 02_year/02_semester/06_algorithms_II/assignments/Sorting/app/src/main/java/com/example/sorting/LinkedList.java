...
import java.util.Random;
    ...
    public class LinkedList<E extends Comparable<E>> {
        ...
        public void generateRandomList(int x){
            Random rand=new Random();
            //init first node
            first=new Node(rand.nextInt(x),null);

            Node node=first;
            Node next=new Node(rand.nextInt(x),null);
            for (int i = 0; i < x-1; i++) {
                node.setNext(next); //apuntam al següent element
                next=new Node(rand.nextInt(x),null);//cream valor aleatori
                node=node.getNext();//avançam posició
            }
        }
