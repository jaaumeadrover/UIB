/*
 * Exemple de fusió asimètrica de dos fitxers emprant la tècnica del centinel·la
 *
 * L'algorisme s'anomena asimètric ja que és un recorregut que dona prioritat
 * a un fitxer
 */
package pkg089_fusioasimetrica;
...
public class FusioAsimetrica {
    ...
    private static void fusiona() {
        try {
            ...
            while (!x1.esCentinela()) {
                while (x2.getCodi() < x1.getCodi()) {
                    f3.writeObject(x2);
                    x2 = (Persona) f2.readObject();
                }
                f3.writeObject(x1);
